...
public class ProductService {
    ...
    public void addProduct(Product p) {
        if (products.containsKey(p.getId())) throw new RuntimeException("Duplicate ID");
        if (p.getStock() < 0) throw new RuntimeException("Negative stock");
    ...
    public List<Product> getLowStockProducts(int threshold) {
        ...
        for (Product p : products.values()) if (p.getStock() <= threshold) list.add(p);
        return list;
    }
    
    public boolean reserveStock(String id, int qty) {
        Product p = products.get(id);
        return p != null && p.reserveStock(qty);
    }
    
    public void releaseReservedStock(String id, int qty) {
        Product p = products.get(id);
        if (p != null) p.releaseReservedStock(qty);
    }
    
    public void commitReservedStock(String id) {
        Product p = products.get(id);
        if (p != null) p.commitReservedStock();
    }
    ...
    public void updateStock(String id, int qty) {
        Product p = products.get(id);
        if (p != null) p.setStock(p.getStock() + qty);
    }
}